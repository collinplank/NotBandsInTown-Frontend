...
const handleSearch = (query, navigate, setSearchParams) => {
  const trimmedQuery = query.trim();
  if (trimmedQuery) {
    setSearchParams({ name: trimmedQuery });
    navigate(`/artists?name=${trimmedQuery}`);
  } else {
    setSearchParams({});
    navigate("/artists");
  }
    ...
    <header className="bg-white shadow-md">
      <nav className="max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleSearch(query, navigate, setSearchParams);
          }}
          className="relative w-full sm:w-96 max-w-lg"
        >
          <input
            type="text"
            name="name"
